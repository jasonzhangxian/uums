...
class User_kpi extends REST_Controller {
    ...
    public function user_kpi_get()
    {
        ...
        if($system_id)
        {
            $data = $this->user_kpi->get_one(array('system_id'=>$system_id));
        ...
        }else
        {
            ...
            $user_kpi = $this->user_kpi->get_all($where,'*','system_id','desc', $limit,$start);

            $this->db->from('admin_user');
            $this->db->select('username');
            $this->db->where('user_id',$user_kpi[0]['update_user_id']);
            $query = $this->db->get()->result_array();

            $total = $this->user_kpi->get_count($where);
            if($user_kpi!==NULL){
                foreach($user_kpi as $n)
                {
                    $records[] = array(
    ...
    public function user_kpi_post(){
        ...
        if ($error === FALSE)
        {
            if($this->user_kpi->save((is_numeric($system_id) ? array('system_id'=>$system_id) : NULL),$data))
            {
     ...
     * @return string
     */
    public function user_kpi_delete(){
        $system_id = $this->delete('system_id');
        if ($this->user_kpi->delete(array('system_id'=>$system_id)))
        {
            $response = array('success' => TRUE, 'feedback' => '成功： 此项操作成功。');
        }
        else
