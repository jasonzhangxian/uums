...
Uums.settings.SidebarPanel = function(app) {
  config = {};
  
  config.title = UumsLanguage.sidebarConfigTitle;
  config.app = app;
  ...
  Uums.settings.SidebarPanel.superclass.constructor.call(this, config);
};

Ext.extend(Uums.settings.SidebarPanel, Ext.Panel, {
  ...
  buildDisplaySettingPanel: function(app) {
	  ...
	  var sidebarSetting = new Ext.Panel({
	    ...
	    items: [
		    {
		      border: false,
		      items: {border: false, html:UumsLanguage.sidebarStateSettingTitle},
		      x: 15,
		      y: 15
		    },
		    {
		    ...
		    {
		      border: false,
		      items: {border: false, html:UumsLanguage.sidebarTransparencySettingTitle},
		      x: 170,
		      y: 16
		    },
		    {
		    ...
		    {
		      border: false,
		      items: {border: false, html: UumsLanguage.sidebarChoseGroundColor},
		      x: 385,
		      y: 15
		    },
		    {
		      border: false,
		      items: this.btnBackgroundColor = new Ext.Button({text: UumsLanguage.btnBackgroundColor, handler: this.onChangeBgColor,scope: this}),
		      x: 390,
		      y: 40
		    }
	    ]
	  ...
	  var pnlDisplaySetting = new Ext.Panel({
	    ...
	    items: {
	      xtype: 'fieldset',
	      title: UumsLanguage.sidebarSettingTitle,
  ...
  onSidebarOpenedCheck: function(checkbox, checked) {
	...
	onShow: function() {
    Uums.settings.SidebarPanel.superclass.onShow.call(this);
    ...
    this.pnlGadgets = new Ext.Panel({  
      ...
      items: {
        xtype: 'fieldset',
	      title: UumsLanguage.sidebarGadgetsTitle,
	      height: 250,
    ...
    this.gadgetsView = new Ext.DataView({
      ...
      emptyText: UumsLanguage.sidebarNoGadgets,
      listeners: {
        render: function() {
