...
Uums.settings.ThemePanel = function(app) {
  ...
  this.store.on('load', function(store, records){
    if(records){
      defaults.setTitle(UumsLanguage.availableTheme + ' (' + records.length + ')');
  ...
  view = new Ext.DataView({
    autoHeight:true,
    emptyText: UumsLanguage.noThemeText,
    itemSelector:'div.setting-view-thumb-wrap',
    loadingText: UumsLanguage.loadingText,
    singleSelect: true,
  ...
  var transparency = new Ext.Panel({
    ...
    height: 70,
    items: [
      {x: 15, y: 15, xtype: 'label', text: UumsLanguage.taskbarTransparency},
      this.slider.slider,
      this.slider.display
    ],
    layout: 'absolute',
  ...
  Uums.settings.ThemePanel.superclass.constructor.call(this, {
    title: UumsLanguage.ThemeSetting,
    layout: 'border',
    labelAlign:'left',
    border: false,
...
Ext.extend(Uums.settings.ThemePanel, Ext.Panel, {
  onShow: function() {
    Uums.settings.ThemePanel.superclass.onShow.call(this);
