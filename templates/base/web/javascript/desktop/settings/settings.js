...
Ext.namespace("Uums.settings");

Ext.define('Uums.settings.SettingsDialog', {
  extend: 'Ext.Window',
  ...
  constructor: function(config) {
    ...
    Ext.apply(config, {
      title: UumsLanguage.DesktopSetting,
      iconCls: 'services',
  ...
  buildPanels: function(app) {
  	this.pnlModule = new Uums.settings.ModulePanel(app);
  	this.pnlBackground = new Uums.settings.BackgroundPanel(app);
  ...
  save: function(app) {
    Ext.MessageBox.show({
      msg: UumsLanguage.saveDataMsg, 
      progressText: UumsLanguage.saveDataProgressText, 
      width:300, 
      wait:true, 
    ...
    Ext.Ajax.request({
      ...
      callback: function(options, success, response){
        if(Ext.decode(response.responseText).success){
          ...
          Ext.MessageBox.hide();
          Ext.MessageBox.alert(UumsLanguage.msgErrTitle, UumsLanguage.connServerFailure);
        }
      },
      failure: function(){
        Ext.MessageBox.hide();
        Ext.MessageBox.alert(UumsLanguage.msgErrTitle, UumsLanguage.lostConnectionToServer);  
      },
      scope: this
    });
  }
