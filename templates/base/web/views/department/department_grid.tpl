...
Ext.define('Uums.department.DepartmentGrid', {
  ...
  constructor: function(config) {
    ...
    config.columns =[<!--此处需要改-->
      { header: '部门名称', dataIndex: 'department_name', width: 80},
      { header: '部门水平', dataIndex: 'departmnt_level', flex:1},
  ...
  onDelete: function(record) {
    var departmentId = record.get('department_id');
    <!--department_id-->
  ...
  onClick: function(view, record, item, index, e) {
    ...
    if (index !== false) {
      ...
      if (action != 'img-button') {
        ...
        var departmentId = record.get('department_id');<!--department_id-->
        
        switch(action) {
          case 'status-off':
          case 'status-on':
            flag = (action == 'status-on') ? 0 : 1;
            this.onAction(module, departmentId, index, flag);
  ...
  onAction: function(action, departmentId, index, flag) {
    Ext.Ajax.request({
      url: Uums.department.request_url,
	  method: 'PUT',
      params: {
