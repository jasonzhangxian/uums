...
Ext.define('Uums.api_list.Api_listGrid', {
  ...
  constructor: function(config) {
    ...
    config.columns =[<!--此处需要改-->
      { header: '接口代码', dataIndex: 'api_string', width: 80},
      { header: '接口名称', dataIndex: 'api_name', flex:1},
      ...
      function(btn) {
        if (btn == 'yes') {
          Ext.Ajax.request({
			...
			method: 'DELETE',
            params: {
                id: api_listId<!--api_list_id: api_listId-->
            },
  ...
  onClick: function(view, record, item, index, e) {
    ...
    if (index !== false) {
      ...
      if (action != 'img-button') {
        ...
        var api_listId = record.get('id');<!--api_list_id-->
        
        switch(action) {
          case 'status-off':
          case 'status-on':
            flag = (action == 'status-on') ? 0 : 1;
            this.onAction(module, api_listId, index, flag);
  ...
  onAction: function(action, api_listId, index, flag) {
    Ext.Ajax.request({
      url: Uums.api_list.request_url,
	  method: 'PUT',
      params: {
        id: api_listId,<!--api_list_id-->
        flag: flag
      },
      callback: function(options, success, response) {
        var result = Ext.decode(response.responseText);
...
/* End of file api_list_grid.tpl */
/* Location: ./templates/base/web/views/api_list/api_list_grid.tpl */
